...
if (class_exists('WP_Widget')) {
	class Plugins_Widget extends WP_Widget {
		...
		function widget($args, $instance) {
			...
			echo '</table>';
			echo '<div style="text-align:right;font-size:10px;padding: 5px 15px;">Last updated: '.date('d.m.Y H:i:s',$instance['updated'])."</div>\n";
		...
		function parseUsingPreg($html,$id){
			$preg = '#<h3>\s+<a href="//wordpress.org/plugins/([^/]+)/">([^<]+)</a>\s+</h3>\s+<p class="active_installs">Active Installs: (.*)[+]?</p>#ismU';
			preg_match_all($preg, $html, $matches, PREG_SET_ORDER);
		...
		function parseUsingDom($html,$id){
			...
			foreach($nlist as $im=>$div){
					if($div->hasChildNodes()){
						...
						preg_match('#Active Installs: ([0-9,]+)#',$dls_text,$m);
						$dls = $m[1];
